...
def main():
    ...
    bucket: dict[tuple[int, int], dict] = defaultdict(
        lambda: {"beleids_q4": None, "aum_q4": None, "rend_quarters": {}}
    )
    ...
    for row in cur.execute(
        "SELECT fund_id, year, aum_euro_bn, beleidsdekkingsgraad_pct, beleggingsrendement_pct "
    ...
    new_rows = 0
    fill_aum = fill_beleids = fill_rend = 0
    rejected_rend = rejected_aum = rejected_beleids = 0

    for (fid, year), b in sorted(bucket.items()):
        # Compute the 3 candidate values + sanity check
        ...
        if all(v is None for v in (beleids, aum_bn, rend)):
            continue
        ...
        if (fid, year) in existing:
            ...
            if not args.dry_run:
                cur.execute("""
                    ...
                    (fund_id, year, aum_euro_bn, beleidsdekkingsgraad_pct, beleggingsrendement_pct)
                    VALUES (?, ?, ?, ?, ?)
                """, (fid, year, aum_bn, beleids, rend))

    if not args.dry_run:
        con.commit()

    print(f"\n[plan]   new (fund,year) rows: {new_rows}")
    print(f"[plan]   fills: aum+={fill_aum}  beleids+={fill_beleids}  rend+={fill_rend}")
    print(f"[plan]   rejected (sanity): aum={rejected_aum} beleids={rejected_beleids} rend={rejected_rend}")
