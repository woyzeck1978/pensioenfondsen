...
def analyze_report(filename):
    ...
    for match in mix_matches:
        ...
        if not equity_val:
            e = re.search(r'aandelen[^\d]{0,80}?(\d{1,2}[,\.]\d{1,2})\s*%?', window)
            if e: equity_val = e.group(1) + "%"
        if not fixed_val:
            f = re.search(r'(vastrentend|obligaties)[^\d]{0,80}?(\d{1,2}[,\.]\d{1,2})\s*%?', window)
            if f: fixed_val = f.group(1) + "%"
        if not real_estate_val:
            r = re.search(r'(vastgoed|onroerend)[^\d]{0,80}?(\d{1,2}[,\.]\d{1,2})\s*%?', window)
...
def main():
    ...
    c = conn.cursor()
    c.execute("SELECT id FROM funds WHERE (dekkingsgraad_pct IS NULL OR beleggingsmix IS NULL OR aum_euro_bn IS NULL OR beleggingsmix LIKE 'Unknown%' OR beleggingsmix LIKE 'Mentions mix but exact % parsed failed%') AND name NOT LIKE '%Zorg en Welzijn%'")
    target_ids = {str(row[0]) for row in c.fetchall()}
    ...
    print(f"Found {len(pdf_files)} problematic reports to re-analyze for costs and mix.")
